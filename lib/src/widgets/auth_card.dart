...
// Project imports:
import 'package:flutsign/src/models/signup_data.dart';
...
import '../providers/sign_messages.dart';
import '../providers/sign_theme.dart';
...
class _LoginCardState extends State<_LoginCard> with TickerProviderStateMixin {
  ...
  Future<bool> _submit() async {
    ...
    final messages = Provider.of<SignMessages>(context, listen: false);
  ...
  Widget _buildEmailField(double width, SignMessages messages, Auth auth) {
    return AnimatedTextFormField(
      ...
      interval: _nameTextFieldLoadingAnimationInterval,
      labelText:
          auth.isLogin ? messages.emailOrUsernameHint : messages.emailHint,
      autofillHints: [
        (auth.isLogin ? AutofillHints.nickname : AutofillHints.email)
      ],
      prefixIcon: Icon(FontAwesomeIcons.solidUserCircle),
      keyboardType:
          auth.isLogin ? TextInputType.text : TextInputType.emailAddress,
      textInputAction: TextInputAction.next,
      onFieldSubmitted: (value) {
        FocusScope.of(context).requestFocus(
            auth.isLogin ? _passwordFocusNode : _usernameFocusNode);
      },
  ...
  Widget _buildPasswordField(double width, SignMessages messages, Auth auth) {
  ...
  Widget _buildConfirmPasswordField(
      double width, SignMessages messages, Auth auth) {
    return AnimatedPasswordTextFormField(
      animatedWidth: width,
      enabled: auth.isSignup,
      loadingController: _loadingController,
  ...
  Widget _buildForgotPassword(ThemeData theme, SignMessages messages) {
    return FadeIn(
  ...
  Widget _buildSubmitButton(ThemeData theme, SignMessages messages, Auth auth) {
  ...
  Widget _buildSwitchAuthButton(
      ThemeData theme, SignMessages messages, Auth auth, SignTheme loginTheme) {
    return FadeIn(
      controller: _loadingController,
      offset: .5,
      curve: _textButtonLoadingAnimationInterval,
  ...
  Widget build(BuildContext context) {
    ...
    final messages = Provider.of<SignMessages>(context, listen: false);
    final loginTheme = Provider.of<SignTheme>(context, listen: false);
    final theme = Theme.of(context);
    final deviceSize = MediaQuery.of(context).size;
          ...
          Container(
            ...
            child: Column(
              ...
              children: <Widget>[
                ...
                SizedBox(height: auth.isLogin ? 0 : 20),
...
class _RecoverCardState extends State<_RecoverCard>
  ...
  Widget _buildRecoverButton(ThemeData theme, SignMessages messages) {
  ...
  Widget _buildBackButton(ThemeData theme, SignMessages messages) {
